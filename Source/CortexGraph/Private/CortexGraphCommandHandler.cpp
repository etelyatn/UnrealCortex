...
TArray<FCortexCommandInfo> FCortexGraphCommandHandler::GetSupportedCommands() const
{
	return {
		...
		FCortexCommandInfo{ TEXT("auto_layout"), TEXT("Auto-arrange nodes in mutable Blueprint graphs for readability. Delegate graphs are readable but not mutable.") }
			...
			.Optional(TEXT("subgraph_path"), TEXT("string"), TEXT("Dot-separated composite subgraph path (e.g. 'BeginPlay.Inner')")),
		FCortexCommandInfo{ TEXT("apply_patch"), TEXT("Preview or apply one typed graph patch as a single verified transaction: reversible mutation, one target compile, authoritative readback and optional explicit persistence. Standalone command: it cannot run inside a rollback-enabled batch because an outer batch cannot undo its compile or save boundary. Limits: max_nodes=64, max_edges=256, max_client_id_length=32, max_request_size_bytes=65536, max_scanned_nodes=2048.") }
			...
			.Required(TEXT("nodes"), TEXT("array"), TEXT("Nodes to create or select, each with client_id, node_class, optional params, tagged defaults and position. node_class accepts a canonical class path or a family identifier. Supported families: CallFunction, VariableGet, VariableSet, Self, DynamicCast, ConstructObject, Event."))
			.Required(TEXT("connections"), TEXT("array"), TEXT("Edges between planned client ids or existing node GUIDs: from and to each carry client_id, node_guid or entry, plus pin"))
			.Optional(TEXT("pin_updates"), TEXT("array"), TEXT("Existing input pin defaults to rewrite, each with node_guid, pin and a tagged default"))
