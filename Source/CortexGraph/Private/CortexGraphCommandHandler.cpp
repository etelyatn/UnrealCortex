...
FCortexCommandResult HandleApplyPatch(const TSharedPtr<FJsonObject>& Params)
{
	...
	if (bDryRun)
	{
		...
		Preview.PruneInventory = FCortexGraphMigrationOps::MakePruneInventory(Prepared.PrunePlan);
		Preview.RetirementInventory = FCortexGraphMigrationOps::MakeRetirementInventory(Prepared.RetirementPlan);
...
TArray<FCortexCommandInfo> FCortexGraphCommandHandler::GetSupportedCommands() const
{
	return {
		...
		FCortexCommandInfo{ TEXT("auto_layout"), TEXT("Auto-arrange nodes in mutable Blueprint graphs for readability. Delegate graphs are readable but not mutable.") }
			...
			.Optional(TEXT("subgraph_path"), TEXT("string"), TEXT("Dot-separated composite subgraph path (e.g. 'BeginPlay.Inner')")),
		FCortexCommandInfo{ TEXT("apply_patch"), TEXT("Preview or apply one typed graph patch as a single verified transaction: reversible mutation, one target compile, authoritative readback and optional explicit persistence. Two mutually exclusive shells share the envelope: the authoring shell (nodes/connections/pin_updates) and the migration shell, which publishes replace_entry (replaces a stale inherited implementation entry while preserving the downstream body), copy_subgraph / move_subgraph (bounded same-asset transfer of a selected node set between two graphs with explicit boundary mappings and both-graph recovery), prune_island (removes only the uniquely owned execution island of one entry, preserving every shared node and every retained-consumer link, with a published partition) and retire_entries (retires a selected set of inherited override events; the only migration allowed on compile-invalid Blueprints). Standalone command: it cannot run inside a rollback-enabled batch because an outer batch cannot undo its compile or save boundary. Limits: max_nodes=64, max_edges=256, max_client_id_length=32, max_request_size_bytes=65536, max_scanned_nodes=2048.") }
			...
			.Optional(TEXT("migration"), TEXT("object"), TEXT("Migration shell selector, one operation per request. replace_entry: {\"op\":\"replace_entry\",\"source\":{\"graph_ref\":{graph_guid,graph_kind?,subgraph_path?},\"entry_node_guid\":\"<guid>\"},\"pin_map\":[{\"entry\":\"input\"|\"output\",\"from_pin\":\"<stale pin>\",\"to_pin\":\"<replacement pin>\"}],\"remove_shadowing_member\":false}; it requires target.implementation. copy_subgraph / move_subgraph bound a same-asset transfer: {\"op\":\"copy_subgraph\"|\"move_subgraph\",\"source\":{\"graph_ref\":{graph_guid,graph_kind?,subgraph_path?},\"node_guids\":[\"<guid>\",...]},\"destination\":{\"graph_ref\":{...}},\"boundary\":[{\"from\":{\"node_guid\":\"<selected source node>\"},\"to\":{\"node_guid\":\"<existing destination node>\"}}]} with a pin per endpoint; every crossing edge of the selection needs exactly one boundary entry. prune_island removes the uniquely owned execution island of one entry: {\"op\":\"prune_island\",\"source\":{\"graph_ref\":{graph_guid,graph_kind?,subgraph_path?},\"entry_node_guid\":\"<event or function entry guid>\"},\"approved_node_guids\":[\"<guid>\",...]} where approved_node_guids is the caller's echo of the removable set the preview publishes (omit it to preview the partition, never send it empty); the preview also publishes shared, blocked and external_edges plus complete and scan counts, and the apply removes only links whose far endpoint is an approved deleted node. retire_entries: {\"op\":\"retire_entries\",\"source\":{\"graph_ref\":{\"graph_guid\":\"<ubergraph guid>\"},\"entry_node_guids\":[\"<override event guid>\",...]},\"approved_node_guids\":[\"<guid>\",...]}; selected entries must be override-style UK2Node_Event nodes. Retirement is the only migration permitted when the target Blueprint is compile-invalid. Every shell refuses a non-empty nodes/connections/pin_updates array; the transfer, prune and retirement shells refuse 'target'."))
