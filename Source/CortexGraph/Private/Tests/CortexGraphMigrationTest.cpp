...
/**
 * A Blueprint-generated declaration: the declaration owner is a Blueprint class instead of a native
 * one, so the explicit target selector resolves the declaration through the Blueprint's generated
 * class while a delegate scoped to that Blueprint resolves it through the Blueprint's skeleton class.
 *
 * The function is authored through the engine path (`FBlueprintEditorUtils::AddFunctionGraph`), which
 * is what flags the compiled declaration `FUNC_BlueprintCallable | FUNC_BlueprintEvent`: callable, so
 * the engine accepts it as a delegate binding target (`UEdGraphSchema_K2::FunctionCanBeUsedInDelegate`),
 * and placeable as an event, so a child asset can override it.
 */
struct FBlueprintDeclarationFixture
{
	UPackage* Package = nullptr;
	UBlueprint* Blueprint = nullptr;

	bool Create(const TCHAR* Name, const TCHAR* FunctionName)
	{
		Package = CreatePackage(*FString::Printf(TEXT("/Game/Temp/%s"), Name));
		Blueprint = FKismetEditorUtilities::CreateBlueprint(
			...
			UBlueprint::StaticClass(), UBlueprintGeneratedClass::StaticClass());
		if (!Blueprint) return false;
		UEdGraph* Graph = FBlueprintEditorUtils::CreateNewGraph(Blueprint, FName(FunctionName),
			UEdGraph::StaticClass(), UEdGraphSchema_K2::StaticClass());
		FBlueprintEditorUtils::AddFunctionGraph<UClass>(Blueprint, Graph, true, nullptr);
		FKismetEditorUtilities::CompileBlueprint(Blueprint);
		// Both class objects must declare the declaration: the generated class the selector resolves
		// and the skeleton class a delegate scope resolves are two pointers to one declaration owner.
		return Blueprint->GeneratedClass && Blueprint->SkeletonGeneratedClass
			&& Blueprint->GeneratedClass->FindFunctionByName(FName(FunctionName)) != nullptr
			&& Blueprint->SkeletonGeneratedClass->FindFunctionByName(FName(FunctionName)) != nullptr;
	}
...
UK2Node_Event* AddEventNode(
	...
	const int32 Y)
{
	return AddEventNode(Graph, FunctionName, FindObject<UClass>(nullptr, OwnerClassPath), X, Y);
}
...
bool FCortexGraphMigrationReferenceInventoryTest::RunTest(const FString& Parameters)
{
	...
	// (g) The selected declaration is generated by a Blueprint. One declaration owner has two class
	// objects: the explicit target selector resolves the declaration through the Blueprint's generated
	// class (`FCortexGraphSymbolResolver::ResolveClass` -> `UBlueprint::GeneratedClass`, then
	// `UFunction::GetOwnerClass`), while a delegate scoped to that same Blueprint resolves it through
	// the skeleton class (`UK2Node_CreateDelegate::GetScopeClass` returns `SkeletonGeneratedClass` for a
	// scope pin typed as a Blueprint-generated class). The two pointers describe the same declaration,
	// so the inventory must normalize the identity before counting: an external binding to a
	// Blueprint-generated declaration is one reference, not zero.
	{
		FBlueprintDeclarationFixture Declarer;
		TestTrue(TEXT("declaring Blueprint declares a bindable function"),
			Declarer.Create(TEXT("BP_MigrationDeclarer_T11"), TEXT("DeclaredPayload")));
		if (!Declarer.Blueprint) { Declarer.Cleanup(); return false; }
		UClass* const DeclaredOwner = Declarer.Blueprint->GeneratedClass;

		FFixture Fixture;
		TestTrue(TEXT("inheriting fixture created"),
			Fixture.Create(TEXT("BP_MigrationDeclared_T11"), DeclaredOwner));
		if (!Fixture.Blueprint) { Declarer.Cleanup(); return false; }
		UEdGraph* const Graph = EnsureEventGraph(Fixture.Blueprint);
		ClearGraphNodes(Graph);
		UK2Node_Event* const StaleEntry = AddEventNode(Graph, TEXT("DeclaredPayload"), DeclaredOwner, 0, 0);
		UK2Node_CallFunction* const Print = AddPrintNode(Graph, TEXT("declared body"), 400, 0);
		LinkNodes(Graph, StaleEntry, TEXT("then"), Print, TEXT("execute"));
		FKismetEditorUtilities::CompileBlueprint(Fixture.Blueprint);

		// The holding asset is a second child of the declaring Blueprint: it binds the inherited
		// declaration and is the external reference the inventory must count.
		UPackage* const HolderPackage = CreatePackage(TEXT("/Game/Temp/BP_MigrationDeclaredHolder_T11"));
		UBlueprint* const Holder = FKismetEditorUtilities::CreateBlueprint(DeclaredOwner, HolderPackage,
			TEXT("BP_MigrationDeclaredHolder_T11"), BPTYPE_Normal, UBlueprint::StaticClass(), UBlueprintGeneratedClass::StaticClass());
		TestNotNull(TEXT("binding asset created"), Holder);
		if (Holder)
		{
			UEdGraph* const HolderGraph = EnsureEventGraph(Holder);
			FEdGraphPinType ScopeType;
			ScopeType.PinCategory = UEdGraphSchema_K2::PC_Object;
			ScopeType.PinSubCategoryObject = DeclaredOwner;
			TestTrue(TEXT("scope member added"),
				FBlueprintEditorUtils::AddMemberVariable(Holder, FName(TEXT("DeclaredTarget")), ScopeType));
			UK2Node_VariableGet* const ScopeGet = AddVariableGetNode(HolderGraph, TEXT("DeclaredTarget"), 0, 0);

			// An editor-valid binding: the delegate out pin is linked to a resolvable, parameterless
			// engine delegate signature (`UAudioComponent::OnAudioFinished`) through the same input pin
			// the engine itself wires a create-delegate node to, so `IsValid()` holds and the selected
			// function survives the engine's own node repair — the state an editor-saved asset keeps.
			UK2Node_AddDelegate* const Bind = NewObject<UK2Node_AddDelegate>(HolderGraph);
			Bind->DelegateReference.SetExternalMember(FName(TEXT("OnAudioFinished")), UAudioComponent::StaticClass());
			Bind->CreateNewGuid();
			Bind->AllocateDefaultPins();
			HolderGraph->AddNode(Bind, true, false);

			UK2Node_CreateDelegate* const Binding = NewObject<UK2Node_CreateDelegate>(HolderGraph);
			Binding->CreateNewGuid();
			Binding->AllocateDefaultPins();
			HolderGraph->AddNode(Binding, true, false);
			TestTrue(TEXT("binding object pin scoped to the declaring Blueprint"),
				LinkPins(HolderGraph, ScopeGet ? ScopeGet->FindPin(FName(TEXT("DeclaredTarget")), EGPD_Output) : nullptr,
					Binding->FindPin(UEdGraphSchema_K2::PN_Self)));
			TestTrue(TEXT("binding delegate pin linked to a resolvable delegate signature"),
				LinkNodes(HolderGraph, Binding, TEXT("OutputDelegate"), Bind, TEXT("Delegate")));
			Binding->SetFunction(FName(TEXT("DeclaredPayload")));
			// The engine's own validity criteria hold for this binding, so its carried function name is
			// the engine's state and not a post-hoc override: the delegate pin resolves to a signature,
			// the scope resolves to the declaring Blueprint, the selected declaration is visible in that
			// scope, matches the signature and is bindable. `UK2Node_CreateDelegate::IsValid`, which
			// aggregates exactly those checks, is not exported from BlueprintGraph, so each criterion is
			// asserted through the exported engine API it is built from.
			UFunction* const BoundSignature = Binding->GetDelegateSignature();
			UClass* const BindingScope = Binding->GetScopeClass();
			UFunction* const Selected = BindingScope ? BindingScope->FindFunctionByName(FName(TEXT("DeclaredPayload"))) : nullptr;
			TestNotNull(TEXT("the binding delegate pin resolves to a delegate signature"), BoundSignature);
			TestTrue(FString::Printf(TEXT("the binding scope is the declaring Blueprint's skeleton class, not its generated class [%s]"),
					*GetNameSafe(BindingScope)),
				BindingScope == Declarer.Blueprint->SkeletonGeneratedClass
					&& BindingScope != Declarer.Blueprint->GeneratedClass);
			TestNotNull(TEXT("the selected declaration is visible in the binding scope"), Selected);
			TestTrue(TEXT("the selected declaration matches the delegate signature"),
				BoundSignature && Selected && BoundSignature->IsSignatureCompatibleWith(Selected));
			TestTrue(TEXT("the selected declaration is bindable"),
				Selected && UEdGraphSchema_K2::FunctionCanBeUsedInDelegate(Selected));
			Binding->HandleAnyChange(true);
			TestEqual(TEXT("the engine's node repair keeps the selected function of an engine-valid binding"),
				Binding->GetFunctionName().ToString(), FString(TEXT("DeclaredPayload")));
		}

		TSharedPtr<FJsonObject> Request = ReplacementRequest(Fixture.Blueprint,
			TEXT("00000000-0000-0000-0000-000000110016"),
			MakeMigration(Graph, StaleEntry, ReceiveBeginPlayPinMap()), TEXT("DeclaredPayload"),
			*DeclaredOwner->GetPathName());
		FCortexGraphPreparedPatch Prepared;
		FCortexCommandResult Error;
		TestTrue(FString::Printf(TEXT("a Blueprint-generated declaration previews: %s"), *Error.ErrorMessage),
			FCortexGraphPatchOps::Preflight(Fixture.Blueprint, Request, Prepared, Error));
		int32 InAssetReferences = -1;
		int32 ExternalReferences = -1;
		if (Prepared.MigrationPlan.IsValid())
		...
		TestEqual(TEXT("the Blueprint-generated declaration has no in-asset binding"), InAssetReferences, 0);
		TestEqual(TEXT("a delegate scoped to the declaring Blueprint counts the generated declaration externally"),
		...
		if (HolderPackage)
		{
			HolderPackage->ClearFlags(RF_Standalone);
			HolderPackage->MarkAsGarbage();
		}
		Fixture.Cleanup();
		Declarer.Cleanup();
	}
